...
export default function JournalList({ entries, filter, onEdit, onDelete }: JournalListProps) {
  ...
  return (
    <div className="space-y-6 max-h-[500px] overflow-y-auto pr-2">
      {filteredEntries.length === 0 ? (
        <div className="text-center py-8 text-black">No entries yet. Start writing!</div>
      ) : (
        filteredEntries.map((entry: JournalEntry, index: number) => (
          <div key={index} className="bg-white rounded-lg p-6 shadow-sm hover:shadow-md transition border-pink-200 duration-200">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-xl font-bold text-black">{entry.title}</h3>
              <span className="text-sm text-black">{entry.createdAt}</span>
            </div>
            <div
